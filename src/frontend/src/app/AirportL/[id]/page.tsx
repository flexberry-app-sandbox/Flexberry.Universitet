...
import useGetAirport from '@/hooks/Airport/useGetAirport';
import useUpdateAirport from '@/hooks/Airport/useUpdateAirport';
...
export default function AirportPageEdit() {
  ...
  const handleNotFound = () => {
    router.push(ROUTES_CONFIG.AIRPORT_L);
  };

  const { data, isLoading } = useGetAirport<IAirportE>({ id, viewName, onNotFound: handleNotFound });
  ...
  const { updateAirportAsync } = useUpdateAirport<IAirportE>(handleSuccess, handleError);

  const handleSave = async (newValue: IAirportE, close: boolean) => {
    setCloseAfter(close);
    await updateAirportAsync({ airport: newValue, viewName: viewName });
  };
