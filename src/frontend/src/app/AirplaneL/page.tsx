'use client';

import { Box } from '@mui/material';

import DataTable from '@/components/DataTable';
import { useNotification } from '@/components/Notification';
import useGetAllAirplane from '@/hooks/Airplane/useGetAllAirplane';
import useDeleteAllAirplane from '@/hooks/Airplane/useDeleteAllAirplane';
import { tView } from '@/enums/tView.types';
import { IAirplaneL } from '@/types/Airplane.types';
import useListState from '@/hooks/useListState';
import useExportList from '@/hooks/useExportList';
import { formatError } from '@/utils/errorsUtils';

export default function AirplanePageList() {
  const viewName: string = 'AirplaneL';

  const {
    resetAll,
    isReady,
    applyFieldSettings,
    handleRowClickWithState,
    handleCreateButtonWithState,
    updateSettings: {
      updatePerPage,
      updateSorting,
      updateVisibleColumns,
      updateColumnResize,
      updateColumnReorder,
      updateFilter,
    },
    params: { page, perPage, sorting, filter, search, setSearch },
  } = useListState<IAirplaneL>('AirplanePageList', {
    perPage: 10,
    columnWidth: {},
    columnHidden: {},
    columnOrder: {},
    columnSort: [],
    columnFilter: {},
  });

  const { showError, showSuccess } = useNotification();

  const { data, isLoading, count } = useGetAllAirplane<IAirplaneL>({
    viewName,
    perPage,
    page,
    sorting,
    filter,
    search,
    enabled: isReady,
  });

  const handleSuccess = () => {
    showSuccess(`Запись удалена.`);
  };

  const handleError = (error: Error) => {
    showError(`Ошибка при удалении настройки: ${formatError(error)}.`);
  };

  const { deleteAllAirplane } = useDeleteAllAirplane(handleSuccess, handleError);

  const handleDelete = (items: IAirplaneL[]) => {
    deleteAllAirplane(items.map((item) => item.id));
  };

  const { exportList } = useExportList({
    viewName: tView.AirplaneL,
    fileName: 'AirplaneL',
    sorting,
    filter,
    search,
  });

  const fields = applyFieldSettings([
    {
      field: 'wingspan',
      title: 'Wingspan',
      filter: true,
      type: 'number',
    },
    {
      field: 'engineCount',
      title: 'Engine count',
      filter: true,
      type: 'number',
    },
    {
      field: 'registrationNumber',
      title: 'Registration number',
      filter: true,
      type: 'text',
    },
    {
      field: 'model',
      title: 'Model',
      filter: true,
      type: 'text',
    },
    {
      field: 'capacity',
      title: 'Capacity',
      filter: true,
      type: 'number',
    },
  ]);

  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', flexGrow: '1' }}>
      <DataTable
        data={data ?? []}
        fields={fields}
        title="Airplane"
        onRowClick={(item) => handleRowClickWithState(item)}
        onCreateButtonClick={() => handleCreateButtonWithState()}
        onDelete={handleDelete}
        filters={filter}
        setFilters={(v) => updateFilter(v)}
        setGlobalFilter={setSearch}
        totalRecords={count}
        rowsPerPage={perPage}
        page={page}
        setPage={(page, perPage) => updatePerPage(page, perPage)}
        multiSortMeta={sorting}
        setSorting={(columns) => updateSorting(columns)}
        lazyLoad={true}
        onChangeVisibleColumns={(columns) => updateVisibleColumns(fields, columns)}
        onColumnResize={(field, width) => updateColumnResize(field, width)}
        onColumnReorder={(fields) => updateColumnReorder(fields)}
        showResetButton={true}
        onResetSettingsClick={resetAll}
        onExportClick={exportList}
        isLoading={isLoading}
      />
    </Box>
  );
}
